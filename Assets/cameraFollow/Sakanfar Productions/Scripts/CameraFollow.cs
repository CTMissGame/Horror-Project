    ...
    public class CameraFollow : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            if (playerBody != null)
            {
                ...
                xRotation = 0f; // รีเซ็ตให้มองตรงเสมอตอนเริ่ม
        ...
        {
            // Get mouse input
            float mouseX = Mathf.Clamp(Input.GetAxis("Mouse X"), -10f, 10f) * mouseSensitivityX * Time.deltaTime;
            float mouseY = Mathf.Clamp(Input.GetAxis("Mouse Y"), -10f, 10f) * mouseSensitivityY * Time.deltaTime;
